from django.core.handlers.wsgi import WSGIRequest
from fpoc.devices import FortiGate, FortiGate_HA, LXC, VyOS, LAN, WAN, Interface, Network, Mgmt
from fpoc.fabric_studio import FabricStudio


class FabricStudioSDWAN(FabricStudio):
    """
    """
    template_folder = 'PoC_SDWAN'
    mpls_summary = '10.71.0.0/16'  # mpls_summary assigned to the WAN of each FGT of this PoC
    password = 'Fortinet123#'  # password assigned to each FGT of this PoC

    # child Mgmt is updated by parent Mgmt, and not the reverse, to prevent updating the parent class definition
    mgmt = Mgmt(gw2='172.16.31.251').update(FabricStudio.mgmt)

    devices = {
        'HUB1': FortiGate(offset=0, nameid='fgt000', name_phy='HUB1',
                            mgmt=Interface('port10', 0, '172.16.31.11/24'),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 12),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.11.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.12.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.13.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.14.1/24', alias='MPLS'),
                            )),

        'HUB1-B': FortiGate(offset=1, nameid='fgt009', name_phy='HUB1-B',
                            mgmt=Interface('port10', 0, '172.16.31.7/24'),
                            HA=FortiGate_HA(mode=FortiGate_HA.Modes.FGCP, role=FortiGate_HA.Roles.SECONDARY,
                                            hbdev=[('port6', 0)], sessyncdev=['port7'],
                                            monitordev=['port1', 'port2', 'port3', 'port4', 'port5'])
                            ),

        'HUB2': FortiGate(offset=2, nameid='fgt001', name_phy='HUB2',
                            mgmt=Interface('port10', 0, '172.16.31.21/24'),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 21),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.21.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.22.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.23.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.24.1/24', alias='MPLS'),
                            )),

        'HUB3': FortiGate(offset=3, nameid='fgt004', name_phy='HUB3',
                            mgmt=Interface('port10', 0, '172.16.31.22/24'),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 34),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.31.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.32.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.33.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.34.1/24', alias='MPLS'),
                            )),

        'HUB4': FortiGate(offset=26, nameid='fgt011', name_phy='HUB4',
                            mgmt=Interface('port10', 0, '172.16.31.10/32'),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 43),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.81.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.82.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.83.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.84.1/24', alias='MPLS'),
                            )),

        'BR1': FortiGate(offset=4, nameid='fgt002',  name_phy='BR1',
                            mgmt=Interface('port10', 0, '172.16.31.31/24'),
                            HA=FortiGate_HA(mode=FortiGate_HA.Modes.FGCP, role=FortiGate_HA.Roles.PRIMARY,
                                         hbdev=[('port6', 0)], sessyncdev=['port7'],
                                         monitordev=['port1', 'port2', 'port3', 'port5']),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 12),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.41.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.42.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.43.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.44.1/24', alias='MPLS'),
                            )),

        'BR1-B': FortiGate(offset=22, nameid='fgt008', name_phy='BR1-B',
                        mgmt=Interface('port10', 0, '172.16.31.32/24'),
                        HA=FortiGate_HA(mode=FortiGate_HA.Modes.FGCP, role=FortiGate_HA.Roles.SECONDARY,
                                       hbdev=[('port6', 0)], sessyncdev=['port7'],
                                       monitordev=['port1', 'port2', 'port3', 'port4', 'port5'])
                           ),

        'BR2': FortiGate(offset=6, nameid='fgt003',  name_phy='BR2',
                            mgmt=Interface('port10', 0, '172.16.31.41/24'),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 21),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.51.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.52.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.53.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.54.1/24', alias='MPLS'),
                            )),

        'BR3': FortiGate(offset=7, nameid='fgt005',  name_phy='BR3',
                            mgmt=Interface('port10', 0, '172.16.31.42/24'),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 34),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.61.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.62.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.63.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.64.1/24', alias='MPLS'),
                            )),

        'BR4': FortiGate(offset=24, nameid='fgt010',  name_phy='BR4',
                            mgmt=Interface('port10', 0, '172.16.31.8/24'),
                            lan=Interface('port5', 0),
                            segments=LAN(
                                lan1=Interface('port5', 0),
                                lan2=Interface('port5', 1),
                                lan3=Interface('port5', 2),
                                lan4=Interface('port5', 3),
                                xtd=Interface('port5', 43),
                            ),
                            wan=WAN(
                                inet1=Interface('port1', 0, '100.64.71.1/24', alias='Internet_1'),
                                inet2=Interface('port2', 0, '100.64.72.1/24', alias='Internet_2'),
                                inet3=Interface('port3', 0, '100.64.73.1/24', alias='Internet_3'),
                                mpls1=Interface('port4', 0, '10.71.74.1/24', alias='MPLS'),
                            )),

        'EXT': VyOS(offset=8, nameid='vyos00',  name_phy='EXT',
                            mgmt=Interface('eth0', 0, '172.16.31.6/24'),
                            ),

        # 'WAN': FortiGate(offset=9, nameid='fgt007',
        #                     mgmt=Interface('port10', 0, '172.16.31.251/24'),
        #                     lan=Interface('port5', 0, ''),
        #                     wan=WAN(
        #                         inet=Interface('port9', 0, '198.18.8.1/24', 'INTERNET'),
        #                         inet1=Interface('port1', 0, '203.0.113.254/24', 'ISP1'),
        #                         inet2=Interface('port2', 0, '198.51.100.254/24', 'ISP2'),
        #                         mpls1=Interface('port3', 0, '10.71.0.254/24', 'MPLS_SP1'),
        #                     )),

        'HUB1-LXC': LXC(offset=11, mgmt=Interface('eth9', 0, '172.16.31.111/24')),
        'HUB2-LXC': LXC(offset=13, mgmt=Interface('eth9', 0, '172.16.31.121/24')),
        'HUB3-LXC': LXC(offset=14, mgmt=Interface('eth9', 0, '172.16.31.122/24')),
        'HUB4-LXC': LXC(offset=27, mgmt=Interface('eth9', 0, '172.16.31.12/24')),
        'BR1-LXC': LXC(offset=15, mgmt=Interface('eth9', 0, '172.16.31.131/24')),
        'BR2-LXC': LXC(offset=17, mgmt=Interface('eth9', 0, '172.16.31.141/24')),
        'BR3-LXC': LXC(offset=18, mgmt=Interface('eth9', 0, '172.16.31.142/24')),
        'BR4-LXC': LXC(offset=25, mgmt=Interface('eth9', 0, '172.16.31.9/24')),
        'INTERNET-SERVER': LXC(offset=23, mgmt=Interface('eth9', 0, '172.16.31.100/24')),
    }

    def __init__(self, request: WSGIRequest, poc_id: int = 0):
        # Go up the parent chain to store the WSGI request, merge class-level devices with instance-level devices
        # and configure device access info (from within studio or from studio public IP)
        super().__init__(request, poc_id)

        # Add password and MPLS summary subnet to each FortiGate
        for device in self.devices.values():
            if isinstance(device, FortiGate):
                device.password = self.password
                if device.wan is not None:
                    device.wan.mpls_summary = Network(self.mpls_summary)
